...
def read_csv_safe(path: str) -> pd.DataFrame:
    ...
    if not p.exists() or p.stat().st_size == 0:
        return pd.DataFrame()
    try:
        return pd.read_csv(p)  # parse_dates is globally ignored by our patch
...
# Forecast (detect columns instead of assuming names)
with c2:
    ...
    if not kpf.empty:
        ...
        st.info("No forecast yet. Use Home → sidebar → Refresh.")
